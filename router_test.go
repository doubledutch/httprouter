...
func TestRouter(t *testing.T) {
	...
	router.Handle("GET", "/user/:name", func(ctx context.Context, w http.ResponseWriter, r *http.Request) {
		...
		// NOTE: Before this was testing Params with DeepEqual. Since ByName is currently only exposed for the context,
		// just compare ByName result.
		expected := "gopher"
		actual := ByName(ctx, "name")
		if expected != actual {
			t.Fatalf("wrong wildcard values: want %v, got %v", expected, actual)
		}
	})
...
func TestRouterAPI(t *testing.T) {
	...
	router := New()
	router.GET("/GET", func(ctx context.Context, w http.ResponseWriter, r *http.Request) {
		get = true
	})
	router.HEAD("/GET", func(ctx context.Context, w http.ResponseWriter, r *http.Request) {
		head = true
	})
	router.OPTIONS("/GET", func(ctx context.Context, w http.ResponseWriter, r *http.Request) {
		options = true
	})
	router.POST("/POST", func(ctx context.Context, w http.ResponseWriter, r *http.Request) {
		post = true
	})
	router.PUT("/PUT", func(ctx context.Context, w http.ResponseWriter, r *http.Request) {
		put = true
	})
	router.PATCH("/PATCH", func(ctx context.Context, w http.ResponseWriter, r *http.Request) {
		patch = true
	})
	router.DELETE("/DELETE", func(ctx context.Context, w http.ResponseWriter, r *http.Request) {
		delete = true
	})
...
func TestRouterChaining(t *testing.T) {
	...
	fooHit := false
	router1.POST("/foo", func(ctx context.Context, w http.ResponseWriter, req *http.Request) {
		fooHit = true
		w.WriteHeader(http.StatusOK)
	})

	barHit := false
	router2.POST("/bar", func(ctx context.Context, w http.ResponseWriter, req *http.Request) {
		barHit = true
		w.WriteHeader(http.StatusOK)
	})
...
func TestRouterOPTIONS(t *testing.T) {
	...
	// custom handler
	var custom bool
	router.OPTIONS("/path", func(ctx context.Context, w http.ResponseWriter, r *http.Request) {
		custom = true
	})
